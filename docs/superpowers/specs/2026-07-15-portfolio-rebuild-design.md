...
### v9.5 (2026-07-18): retina canvas fix — pin CSS size on both canvases

Pedro on a DPR-2 display: the glyph field rendered 2x zoomed (only the top-left quadrant visible) and the pointer wake landed far from the cursor. Root cause: the field and flap canvases had backing stores of `viewport x dpr` but no explicit CSS size, and `inset` does NOT stretch replaced elements — an absolutely/fixed positioned `<canvas>` with only insets renders at its intrinsic (attribute) size. At DPR 1 attribute size coincidentally equals the viewport, which is why every probe and screenshot passed; at DPR 2 the canvas displayed at twice the viewport and the splat math (clientX/CW in CSS px) no longer matched what was on screen. Fix: `sizeFlap`/`sizeField` now set `style.width/height` to the CSS pixel size alongside the attribute sizing (the standard canvas-DPR pattern). Verified with `deviceScaleFactor: 2` probes: canvas `getBoundingClientRect()` equals the viewport, field density matches DPR 1, wake renders under the pointer path, mid-drag corner curl correct. Probe lesson recorded: canvas-heavy UI must be probed at DPR 2, not just DPR 1.

### v9.6 (2026-07-18): fail-visible reveal, full overlay teardown, Activity layout A + stamp cascade

Pedro reported the rail column vanishing after a dog-ear click on his dev server. Not reproducible on a clean production build under 7 interaction variants at DPR 1 and 2; diagnosis: the agent's verify harness had been running `rm -rf .next` and production builds inside the SAME directory as Pedro's live dev server, corrupting served chunks. The rail was the one element whose visibility depended on a CSS keyframe completing (`.reveal` base `opacity: 0` + `rise` to-only keyframes) — a lost keyframe left it permanently invisible while JS-driven `[data-reveal]` sections survived. Hardening shipped anyway: (1) `.reveal`/`fade-only` are now from-based keyframes with `fill: both` and a visible base — if the animation never runs, content shows instead of hiding (fail-visible); (2) Proof Pull's unmount cleanup resets plate/flap/dog-ear state (`data-on`, `data-open`, `data-peeling`, aria) — a dev Fast Refresh mid-plate-view used to orphan the white dog-ear over the normal page (visible in Pedro's screenshot). Harness rule: verification builds now run in an rsynced mirror with symlinked node_modules; the user's `.next` is never touched.

Activity section per Pedro ("better organize", picked layout A; "deserves a better animation" than the sweep): number and caption share one baseline; the stats bar and the two caption rows merge into a single meta row (`streak N days · busiest N · weeks N/26` left, `last 6 months` right); the print-head sweep is replaced by a STAMP CASCADE — each dot pops at `col*22ms + row*9ms` over 240ms with easeOutBack (~8% overshoot), total ~844ms, odometer rolls in sync. Reduced motion unchanged (instant plate). Verified: cascade frames at 160/460/1060ms, meta row settles, swell/tooltip/dog-ear flows clean at DPR 2, zero console errors.

### v9.7 (2026-07-21): scroll lock on html only — body overflow hidden broke the sticky rail

Pedro: the dog-ear error "is now happening when i click it after i scroll to the bottom of the site," and the pull is buggy. Reproduced on a clean production build at DPR 2: mid-drag at scroll bottom, the entire rail column went white. Isolation probe (each ingredient applied alone at scroll bottom) proved the trigger was the scroll lock itself, before any clip-path: when `<html>` has a non-visible overflow, that value propagates to the viewport and `<body>`'s overflow stops propagating — `body { overflow: hidden }` then makes body ITSELF a scroll container. The rail (`md:sticky md:top-0`) pins to its nearest scroll container, which becomes body; body's own scrollTop is forever 0, so the rail falls back to its static position at document top — exactly `scrollY` pixels above the viewport. At scroll top the static and stuck positions coincide, which is why every earlier probe (all run at scroll 0) passed. Fix: `lockScroll` sets `overflow: hidden` on `documentElement` only; body content does not overflow body, so nothing becomes scrollable, and the viewport stays the rail's scrollport with its offset intact. Verified at DPR 2: rail pinned at y=0 through mid-drag, spring, plate view, and restore at scroll bottom; wheel dead during drag and plate view, alive after exit; top-of-page pull unchanged; zero page errors. Probe lesson recorded: gesture UI must be probed at multiple scroll offsets, not just scroll top.

### v9.8 (2026-07-22): release safety net + anisotropic corner box

Pedro on production (pedroernesto.me): the sheet froze mid-peel after he released the button, and dragging the peel up to the top "kinda breaks"; standing goal set to "PERFECT, no weird interactions."

Frozen peel: `drag` is the only mode that exits via an event; a 9-variant release battery (flick, veto, chord, pointercancel, edge release, mid-spring re-grab) all completed with `pointerup` delivered, so the real-world failure is a SWALLOWED release — e.g. a native context menu from a two-finger tap mid-drag (`contextmenu` targets the hit-tested element, not the capture target, so the dog-ear's own preventDefault never sees it), or the OS stealing capture. Fix is belt-and-suspenders, all routed through the same idempotent `onPointerUp` (mode/pid guards make double delivery harmless): (1) `pointermove` with `buttons === 0` mid-drag resolves the drag — recovery on the first mouse wiggle; (2) `lostpointercapture` on the dog-ear resolves it; (3) window-level capture-phase `pointerup`/`pointercancel` backstop; (4) window-level `contextmenu` is suppressed only while dragging, so the menu can no longer swallow the release. Verified: 6 recovery scenarios + the 9-variant battery all settle; context menu untouched at rest.

Drag-to-top slab: with the square corner box (`L = 4d+80`) an edge-hugging pull lays the fold line parallel to an edge and the box opens to full width, so a straight-up drag covered the entire viewport with featureless paper-back (reads as the page breaking). The box is now ANISOTROPIC: `Lx = 4|dx| + 0.5|dy| + 80`, `Ly` mirrored, where (dx,dy) is the pull vector. Diagonal pulls keep the classic corner wedge and the full-sheet spectacle; edge-hugging pulls peel a narrow strip along the edge, like lifting tape, with page content readable throughout. During the opening whip the box widens by `8×` the excess distance past the commit point (`openD0`), so the exit consumes the whole sheet instead of freezing at the drag-time strip. Verified at DPR 2 with screenshot review: straight-up strip peel, left-edge strip, deep diagonal spectacle unchanged, direction reversal, circular drags, corner jitter, both whip exits; V1–V9 + R1–R6 + scroll-lock rail probes all pass.

### v9.9 (2026-07-22): fold angle clamp — the peel is always the corner wedge

Pedro with before/after screenshots: the peel "shouldn't be able to break like this [floating rectangle with straight cut edges] — it should always be like this [corner curl with diagonal crease]." The v9.8 anisotropic box produced rectangle-ish patches whose straight box edges read as CUT paper. Root insight: every broken look traces to the fold line lying near-parallel to a page edge (its edge intercept `d/(2|dir_axis|)` diverges as the pull goes vertical/horizontal). Fix: clamp the pull ANGLE to a sector, `FOLD_MIN 20° – FOLD_MAX 70°` from horizontal — the pointer dictates distance faithfully but the fold direction saturates, like real glued paper whose crease refuses to lie flat along an edge. With the sector, the lifted region is ALWAYS a corner triangle at any drag distance and the corner box became dead code: deleted entirely (with its whip-widening `openD0`), replaced by a plain viewport clip on `lifted` (stops reflections of below-viewport paper from ghosting back on screen at shallow angles — likely part of Pedro's floating-slab screenshot). Deep diagonal pulls keep the full-sheet spectacle (the clamp is inert there). Verified at DPR 2: straight-up and bottom-edge drags now produce the exact reference corner curl; top-center pull's clip is a pure triangle; sweep (reversal, circle, jitter, both whip exits) clean; V1–V9, R1–R6, and scroll-lock rail probes all pass.

### v9.10 (2026-07-23): mobile polish — container-query columns, fnav hairline, atomic stat pairs

Pedro: "make it look better on mobile, its slightly off." Audit at 390/360/700/768/844/1440 (DPR 2–3, per-screenful screenshots) found three defects, none phone-exclusive:

1. **Date columns crushed the content at tablet widths.** The timeline/accordion grids switched to `[10rem_1fr]` on the viewport `sm:` breakpoint, but with the md rail (19rem + 4rem gap) the content column at 768–950px is only ~350–460px wide — titles became 3-line towers ("Software Engineer, / Maestro / (mobile.dev)"), exactly the iPad-portrait band. The content column is now a `@container` and every inner grid (timeline rows, accordion rows, the name/email form pair) uses the `@lg:` (32rem) container variant instead: the split now tracks the COLUMN's real width. Phone (<640) unchanged, 640–767 unchanged (column ≥ 592px), 768–~950 now stacks, ≥ ~950 and desktop unchanged (verified computed grids `160px 456px` / form `312px 312px` at 700 and 1440 — identical to before).
2. **The floating nav had no edge.** Translucent material (0.82 + blur) with no bottom border: over white gaps the bar was invisible, with nav text floating 40px above section text; passing content smudged under it with no boundary. Added a 1px `var(--border)` bottom hairline (the site's hairline language; the `prefers-contrast: more` solid #111 border still overrides it).
3. **The activity stats row wrapped mid-pair** at ≤390px ("WEEKS" on one line, "20/26" alone on the next). Each label+value pair is now an atomic `inline-flex` unit with its trailing `·` separator riding along, so wraps fall only BETWEEN stats; `streak 1` also now reads "1 day", not "1 days".

Also ruled out (not bugs): the sparse left-clustered dot grid in probes is stale ISR snapshot data (the live swap fills it — desktop probe showed 849 with a full-width plate); the clipped big number is the odometer mid-roll (~844ms stamp sync). Verified: no horizontal overflow at 360/390, proof-pull sweep + recovery suites green, zero page errors at all six viewports.
